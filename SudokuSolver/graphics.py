...
class Color(object):
    """
    Author: nurrencd
    written for adding colors to a grid
    """
    def __init__(self, r, g, b, weight=1):
        self.r = float(r) * weight
        self.g = float(g) * weight
        self.b = float(b) * weight
        self.weight = weight
    ...
    def getHexString(self):
        """
        Returns the hex string in format '#rrggbb'
        """
        rVal = 255 * self.r / self.weight
        gVal = 255 * self.g / self.weight
        bVal = 255 * self.b / self.weight
        return '#' + hex(int(rVal))[2:4] + hex(int(gVal))[2:4] + hex(int(bVal))[2:4]
